...
This model1 version implements different depr rates for capital and investment
'''
...
def run_model(timeswitch, weight, depr, reg =0):  # Do not change reg = 0 here!
    ...
    model.pm_delta_kap = pyo.Param(initialize=0.02)  # default = 0.05 (here used for investment only)
    model.pm_delta_cap = pyo.Param(initialize=0.05) # here used for capitl only
    ...
    def capital_constraint_rule(m,t):
        if m.pm_tall_val[t] > m.pm_firstyear:
            return m.vm_cesIO[t] == (1 - m.pm_delta_cap) ** m.pm_dt[t] * m.vm_cesIO[t - 1] + m.pm_cumdepr_old[t] * m.vm_invMacro[t - 1] + m.pm_cumdepr_new[t] * m.vm_invMacro[t]
        else:
...
if __name__ == "__main__":
    ...
    for t in Ti:
        ...
        plt.tight_layout()
        plt.savefig(f"C:\\Users\\mikae\\Documents\\Uni\Project 1\\report\\ramseyreport\\Outcomes_new\\Results_deprdif_t{t}_wide.png", dpi=500)
        plt.show()
