...
for i in range(ABs.shape[1]):
    ...
    for t in Ti:
        ...
        if __name__ == "__main__" and t != 1:
            ...
            np.random.seed(1)
            Problem = Ramsey()
            x0 = np.append(np.append(np.asarray(pm_welf)[0:17], [pm_cumdepr_new_array[1], pm_cumdepr_new_array[16]]),  [pm_cumdepr_old_array[1], pm_cumdepr_old_array[16]])
