...
def f_tall_string(m):
    ...
    if m.time == 5:
        return f_tall_string_e()
    if m.time == 6:
        return f_tall_string_f()
    if m.time == 7:
        return f_tall_string_g()
    if m.time == 8:
        return f_tall_string_h()
...
def f_tall_string_e():
    return ['2020', '2021', '2022', '2023', '2024', '2025', '2026', '2027',
            ...
            '2044', '2045', '2046', '2047', '2048', '2049', '2050', '2051',
            '2052', '2053', '2054', '2055', '2056', '2057', '2058', '2059',
            '2060', '2061', '2062', '2063', '2064', '2065',  '2071', '2072', '2073', '2074', '2075',
            '2076', '2077', '2078', '2079', '2080', '2081', '2082', '2083',
            '2084', '2085', '2086', '2087', '2088', '2089', '2090', '2091',
            '2092', '2093', '2094', '2095', '2096', '2097', '2098', '2099',
            '2100', '2101', '2102', '2103', '2104', '2105', '2106', '2107',
...
def f_tall_string_f():
    return ['2020', '2021', '2022', '2023', '2024', '2025', '2031', '2032', '2033', '2034', '2035',
            '2036', '2037', '2038', '2039', '2040', '2041', '2042', '2043',
            '2044', '2045', '2046', '2047', '2048', '2049', '2050', '2051',
            '2052', '2053', '2054', '2055', '2056', '2057', '2058', '2059',
            '2060', '2061', '2062', '2063', '2064', '2065', '2066', '2067',
...
def f_tall_string_h():
    return ['2020', '2021', '2022', '2023', '2024', '2025', '2027',
            '2028', '2029', '2030', '2031', '2032', '2033', '2034', '2035',
            '2036', '2037', '2038', '2039', '2040', '2041', '2042', '2043',
            '2044', '2045', '2046', '2047', '2048', '2049', '2050', '2051',
            '2052', '2053', '2054', '2055', '2056', '2057', '2058', '2059',
...
def f_cumdepr_old(m, t):  # switch function for old
    ...
    if m.depr == 5:  # Experimentation factor!
        if m.pm_dt[t] == 1:
            return 5 * f_cumdepr_old_3(m, t)
        if m.pm_dt[t] == 5:
            return 5 * f_cumdepr_old_3(m, t)
        if m.pm_dt[t] == 10:
            return f_cumdepr_old_3(m, t)
...
def f_cumdepr_new(m, t):  # switch function for new
    ...
    if m.depr == 5:  # Experimentation factor!
        if m.pm_dt[t] == 1:
            return 5 * f_cumdepr_new_3(m, t)
        if m.pm_dt[t] == 5:
            return 10 * f_cumdepr_new_3(m, t)
        if m.pm_dt[t] == 10:
            return f_cumdepr_new_3(m, t)
...
def f_cumdepr_old_1(m, t):  # TODO: Does this work for dt = 10? Why + 0.5??
    pm_cumDeprecFactor_old = ((1 - m.pm_delta_kap) ** (m.pm_dt[t] / 2 + 0.5) - (1 - m.pm_delta_kap) ** m.pm_dt[
        t]) / m.pm_delta_kap
...
def f_cumdepr_old_2(m, t):
    ...
    dt = m.pm_dt[t]
    if (dt % 2) == 0:
        pm_cumDeprecFactor_old = ((1 - m.pm_delta_kap) ** (m.pm_dt[t] / 2) - (1 - m.pm_delta_kap) ** (
        m.pm_dt[t])) / m.pm_delta_kap - 1 / 2 * (1 - m.pm_delta_kap) ** (m.pm_dt[t] / 2)
        return pm_cumDeprecFactor_old
    # if dt[t] is uneven:
    else:
        pm_cumDeprecFactor_old = ((1 - m.pm_delta_kap) ** (m.pm_dt[t] / 2 + 0.5) - (1 - m.pm_delta_kap) ** (
        m.pm_dt[t])) / m.pm_delta_kap
    return pm_cumDeprecFactor_old
...
def f_cumdepr_new_2(m, t):
    dt = m.pm_dt[t]
    if (dt % 2) == 0:
        pm_cumDeprecFactor_new = (1 - (1 - m.pm_delta_kap) ** (m.pm_dt[t] / 2)) / m.pm_delta_kap - 1 / 2 * (
                    1 - m.pm_delta_kap) ** (m.pm_dt[t] / 2)
        return pm_cumDeprecFactor_new
    else:
        pm_cumDeprecFactor_new = (1 - (1 - m.pm_delta_kap) ** (m.pm_dt[t] / 2 + 0.5)) / m.pm_delta_kap
        return pm_cumDeprecFactor_new
...
def f_cumdepr_old_3(m, t):
    pm_cumdeprFactor_old = (1 - m.pm_delta_kap - (1 + m.pm_delta_kap * (m.pm_dt[t] - 1)) * (1 - m.pm_delta_kap) **
                            m.pm_dt[t]) / (m.pm_delta_kap ** 2 * m.pm_dt[t])
    return pm_cumdeprFactor_old


def f_cumdepr_new_3(m, t):  #
    pm_cumDeprecFactor_new = ((1 - m.pm_delta_kap) ** (m.pm_dt[t] + 1) + m.pm_delta_kap * m.pm_dt[
        t] + m.pm_delta_kap - 1) / (m.pm_delta_kap ** 2 * m.pm_dt[t])
    return pm_cumDeprecFactor_new


# Second version of linear regression (0.5*(1-d)* cumdepr_old * 0.5*cumdepr_new)
def f_cumdepr_old_4(m, t):
    pm_cumDeprecFactor_old = ((m.pm_delta_kap * ((m.pm_delta_kap - 2) * m.pm_dt[t] + 2) - 2) * (1 - m.pm_delta_kap) **
                              m.pm_dt[t] - 2 * m.pm_delta_kap + 2) / (2 * m.pm_delta_kap ** 2 * m.pm_dt[t])
    return pm_cumDeprecFactor_old


def f_cumdepr_new_4(m, t):
    pm_cumDeprecFactor_new = (2 * (1 - m.pm_delta_kap) ** (m.pm_dt[t] + 1) + 2 * m.pm_delta_kap * (1 + m.pm_dt[t]) - (
        m.pm_delta_kap) ** 2 * m.pm_dt[t] - 2) / (2 * m.pm_delta_kap ** 2 * m.pm_dt[t])
    return pm_cumDeprecFactor_new
