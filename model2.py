...
def run_model(timeswitch, weight, depr, reg = 1):  # Do not change reg = 1 here!
    ...
    def cuminv_constraint_rule(m,t):
        if t == 0:
            ...
            dts = range(1, m.pm_dt[t] + 1)
            return m.vm_cuminv[t] == sum(np.asarray([(1-m.pm_delta_kap)**dts[i] *  m.vm_invMacro[t - 1] + (m.vm_invMacro[t] - m.vm_invMacro[t - 1]) / m.pm_dt[
                t] * dts[i] for i in range(0, len(dts))]))
    ...
    #model.inv_lin_Constraint = pyo.Constraint(model.Tall, rule= inv_lin_rule)
